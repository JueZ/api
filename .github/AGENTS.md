...
- Keep exact-main CI mandatory, but allow the trusted post-merge controller to skip environment deployment for an authenticated complete PR file list only when every current and previous rename path matches the exact protected runtime-neutral allowlist. Documentation, scoped instructions, agent-task definitions, and dedicated non-shipped agent-learning/evaluation code may be allowlisted; workflow, policy, package, application, contract, infrastructure, ambiguous, and mixed changes remain deployment-required.
