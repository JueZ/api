...
- `Delivery v2` owns normal delivery from a protected-main push. Runtime-neutral changes finish without an application build or environment mutation; deployment-impacting changes build once and promote the same accepted digest. Never enable a second controller that can promote the same SHA.
- `Repair and Learning Queue` creates one sanitized issue per stable failure fingerprint and exact-head recurrence. It cannot invoke Codex, copy logs, or rewrite repository files; the initiating Codex task owns bounded repair through terminal delivery.
