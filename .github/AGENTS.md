...
- Keep exact-main CI mandatory, but allow the trusted post-merge controller to skip environment deployment for an authenticated complete PR file list only when every current and previous rename path matches the exact protected runtime-neutral allowlist. Ambiguous or mixed changes remain deployment-required.
