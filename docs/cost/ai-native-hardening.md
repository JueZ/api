...
High-risk PR review is cost-bounded independently from runtime REC. Free deterministic CI, policy, and CodeQL checks must pass before the paid call. Serialized workflow runs atomically create one durable repository/PR/head-SHA claim immediately before any request. Reruns and repeated PR/label/manual events reuse a valid completed approved artifact or fail closed; they cannot acquire a second claim for the same head. The trusted controller retains the high-assurance `gpt-5.6-sol` reviewer with high reasoning, but limits a newly claimed head to at most 40,000 diff bytes, 1,500 output tokens, one API call, no SDK retries, and a conservative pre-call ceiling of $0.31. The sanitized artifact records the maximum budget and returned token usage. A missing, malformed, oversized, over-budget, or incomplete review fails closed without an automatic paid retry. Local/unit/API tests use injected fakes and do not enable live review calls.
