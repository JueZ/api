...
High-risk PR review is cost-bounded independently from runtime REC. Free deterministic CI, policy, and CodeQL checks must pass before the paid call and are revalidated both when the durable repository/PR/head-SHA claim is created and immediately before the request. Reruns and repeated PR/label/manual events cannot acquire a second claim for the same head. Approval reuse additionally requires the pinned GitHub Actions App and main controller workflow ID/path/ref/event, a successful first-attempt source run, and a unique exact-head artifact whose API-reported SHA-256 digest matches the evidence sealed into the claim check. Anything else fails closed without another request. The trusted controller retains the high-assurance `gpt-5.6-sol` reviewer with high reasoning, but limits a newly claimed head to at most 40,000 diff bytes, 1,500 output tokens, one API call, no SDK retries, and a conservative pre-call ceiling of $0.31. The sanitized artifact records the maximum budget and returned token usage. A missing, malformed, oversized, over-budget, or incomplete review fails closed without an automatic paid retry. Local/unit/API tests use injected fakes and do not enable live review calls.
