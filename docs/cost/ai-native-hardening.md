...
Pull-request governance makes no OpenAI request. High-risk paths retain deterministic classification and proportional validation, while the protected controller requires exact-head CI, Policy Check, CodeQL, workflow-integrity, permission, secret-access, program-evidence, and complete-rollup proof. Only the two controller jobs that publish eligibility or the stable aggregate receive `checks: write`; deterministic governance itself has read-only check access. Local and CI governance tests therefore have no provider cost and do not depend on API credit availability.

Configure an OpenAI project hard spend limit and earlier alerts in the Platform as the account-level backstop for the optional runtime analyzer. Batch locally validated changes before pushing to reduce CI load and repair churn, not to ration model-backed PR reviews.
