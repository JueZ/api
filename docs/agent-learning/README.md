...
The trusted verifier binds the open PR, exact head, first-attempt controller run, newly consumed review claim, four exact-head aggregates, implementation merge, first-attempt delivery runs, GitHub deployments and jobs, and release-ledger artifacts. Phase 2 is additionally pinned in protected verifier code to implementation PR #349 and its exact base, branch, reviewed head, and merge commit, so internally consistent evidence from an unrelated successful delivery cannot substitute for the reviewed implementation. Paginated history reads require the complete latest exact-head check and legacy-status rollup plus the canonical latest applicable workflow and deployment records; a later pending, failed, cancelled, or otherwise non-passing result cannot be hidden by selecting an earlier success. A historical deployment may end in GitHub's tightly checked `inactive` supersession state only when no later failure exists and fresh test/production deployment plus live-health reads independently prove the exact current protected-main commit. Artifact bytes must match both authenticated GitHub metadata and the recorded SHA-256 digest before the single canonical ledger entry is parsed. Live health reads use the exact repository-owned test and production Function hosts, reject aliases, credentials, ports, paths, queries, and fragments, and disable redirects. The verifier emits only a sanitized result artifact. Phase acceptance still requires an ordinary protected evidence PR; the verifier cannot rewrite the program or learning controls.
