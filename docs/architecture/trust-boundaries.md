...
| MCP client     | Host, forwarded host/scheme, Origin, JSON-RPC, tool arguments | Canonical HTTPS resource origin, exact host/origin checks, per-tool OAuth scope, bounded Zod schemas                 |
| Provider APIs  | Status, headers, JSON/HTML/text, redirects                    | Fixed provider hosts/routes, timeouts, size limits, unknown-boundary parsing, normalized DTOs, sanitized errors      |
