...
| GitHub PR      | Branch code, workflow changes, check names/status                            | Native exact-head squash auto-merge, protected branch rules, required `PR Gate` and `Security Gate` from GitHub Actions, deterministic fail-closed policy    |
