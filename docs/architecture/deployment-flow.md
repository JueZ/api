...
5. `Codex Main Delivery` explicitly dispatches test deployment after main CI. Privileged test, production, rollback, storage-preparation, and manual auto-merge entry points use typed `repository_dispatch` events, which execute only the workflow generation on the default branch; callers cannot select another workflow ref.
...
Credential-bearing smokes use the Azure-discovered Function default hostname. A configured production/canary base URL is accepted only when it exactly matches that discovered origin. No local shell deployment or mutable branch/tag artifact is accepted. The live repository ruleset and Entra/Azure role assignments remain bootstrap prerequisites and cannot be proven by local validation.
