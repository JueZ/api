...
- The official Codex GitHub integration supports review triggers but not an unattended implementation callback. The initiating Codex task must monitor and repair through terminal delivery; failures that outlive it remain deduplicated `codex-repair` issues for the next task.
