...
- The focused repair loads `dist/index.js`, executes `assertRuntimeSafety()` before registration, and independently rejects disabled authentication outside local development. Local regression coverage and the full 258-test suite pass; live test deployment still must prove the behavior.
