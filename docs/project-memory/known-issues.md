...
- Reddit short-share retrieval can be provider-blocked. Exhaustive snapshots expire after 24 hours and have configurable 100,000-comment/96 MiB resource caps; either condition requires a new crawl, and a cap stop is explicitly incomplete. Add a general job/export framework only if telemetry demonstrates a need.
