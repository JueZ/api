...
## Identical-compare verifier compatibility is repaired; durable learning remains
...
- PR #375 merged the bounded compatibility fix as `f7ec8cbb4d9bc44fecd9a833c3d7cc483f6974f2`: omitted `head_commit` is accepted only when status, base, merge base, exact endpoints, ahead/behind counts, stable main refs, and canonical URL prove the fully identical case. Non-identical comparisons retain exact head binding and fail closed. Exact-main CI, test and production delivery, smoke, telemetry, release ledgers, and fresh runtime truth passed.
- Issue #374 remains open because operational recovery is not the versioned learning artifact. It requires an ordinary protected PR with the recurrence fingerprint, executable regression reference, and counterfactual proof. Phase 2 remains `in_progress` until the public-safe evidence PR and later acceptance ledger both complete their required evidence order.

## Phase 2 acceptance still lacks exercised protected evidence

- PR #349 implemented and delivered the versioned-learning system, but its public-safe acceptance evidence has not yet exercised the merged trusted verifier on a protected PR.
