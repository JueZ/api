...
- Fix: PR #147 changed Angular bootstrap to detect MSAL auth-code/error responses in hidden iframes and leave the frame unbootstrapped so the parent MSAL monitor can process the response hash.
- Status: Deployed to production by Promote Production run `25984979189`; smoke tests passed. If a browser tab still has stale MSAL state, sign out/sign in and retry a protected operation.
