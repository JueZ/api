...
- Open and monitor the library-only successor from `codex/agent-learning-trusted-verifier-v4`. Its implementation commit `f3e3b89c6ec564963cc6a8c5367db72a3028626d` is not the later exact reviewed head. It preserves tokenless PR CI, fixed candidate paths, exact implementation identity, complete latest histories, reviewed workflow bytes, server-associated ledger artifacts/digests, fixed runtime hosts, disabled redirects, bounded reads, and the existing merge/delivery controls.
- The successor now rejects ambiguous in-PR final-head claims, binds checkout/option to runtime `GITHUB_WORKFLOW_SHA`, and authenticates that SHA as stable protected-main history while retaining the separate candidate-head binding. Do not mark Phase 2 accepted or wire credentials in this library PR.
