...
- Finish Phase 4 on branch `codex/agent-task-evals-phase-4`: validate the four exact historical tasks, trusted invariant scorers, detached-worktree/context baselines, fake-adapter scoring, timeout process-group termination, cleanup, path/command injection, environment isolation, sanitization, and fixed-path CI integration. Deliver one protected `[skip deploy]` PR; do not run a model under the operator's general-evaluation restriction and record that optional run as blocked. After merge, deliver one small evidence-only PR that marks Phase 3 and Phase 4 accepted against registrations already present on protected `main`; then begin Phase 5 memory freshness and reporting.
