...
7. `CODEX_SECURITY_API_KEY` is configured, and Ubuntu 22.04 run `30745450036` proved scanner file writing before its former `$3` budget was exhausted. Keep setup calls at the operator-selected `$0.10` Luna/low threshold and rely on the locked install, sandbox probe, official sealed-example SARIF export, trusted-base knowledge extraction, and dry run for no-model-cost integration validation. Do not claim a repository verdict unless a live scan actually seals. Before sampling approximately 10–20 advisory scans, obtain explicit approval for a completion-capable budget and effort, then measure cost, runtime, completeness, deferred surfaces, false positives, and sealing reliability; compare representative scans with Sol/xhigh before considering `--fail-on-severity high`, a `gpt-5.6-terra --effort high` low-risk lane, or a separate scheduled/manually dispatched deep scan.
