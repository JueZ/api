...
- Finish Phase 4 on branch `codex/agent-task-evals-phase-4`: validate the four exact historical tasks, trusted invariant scorers, detached-worktree/context baselines, fake-adapter scoring, timeout process-group termination, cleanup, path/command injection, environment isolation, sanitization, and fixed-path CI integration. Deliver one protected `[skip deploy]` PR; do not run a model under the operator's general-evaluation restriction and record that optional run as blocked. Phase 5 will record Phase 4 terminal evidence while adding memory freshness and daily status reporting.
