...
- Deliver the current ledger-only PR first. Then create a protected trusted-primitives module and deterministic tests as one independently useful PR, targeting at most 140,000 diff bytes. Follow with a separate Phase 2 evidence schema/collector/orchestrator PR that imports those merged primitives and stays under the same target.
