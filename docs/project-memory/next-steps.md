...
- Deliver the current ledger-only PR first. Then create one fresh library-only successor from protected main and reapply the reviewed #362 verifier without stale in-PR status claims. Preserve tokenless PR CI, fixed candidate paths, exact implementation identity, complete latest histories, reviewed workflow bytes, server-associated ledger artifacts/digests, fixed runtime hosts, disabled redirects, bounded reads, and the existing merge/delivery controls.
- Add executable prevention for both final #362 findings: in-repository records must call `739cc1d622d2df1446d3a11b594d08d3d268230d` an implementation commit rather than the unknowable later reviewed head; the verifier must bind checkout and option to runtime `GITHUB_WORKFLOW_SHA` and authenticate that SHA as protected-main history while retaining the separate candidate-head binding. Do not mark Phase 2 accepted or wire credentials in the foundation.
