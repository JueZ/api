...
- The successor now rejects every self-recorded open-PR commit identity in changed governance ledgers, binds checkout/option to runtime `GITHUB_WORKFLOW_SHA`, and authenticates that SHA as stable protected-main history while retaining the separate candidate-head binding. Exact PR and commit identities must be archived later from authenticated evidence. Do not mark Phase 2 accepted or wire credentials in this library PR.
