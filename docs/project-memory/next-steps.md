...
7. `CODEX_SECURITY_API_KEY` is configured. Review approximately 10–20 advisory Luna/high PR scans and measure estimated cost, runtime, coverage completeness, deferred surfaces, false positives, and result-sealing reliability. Compare representative scans with Sol/xhigh before treating the lower-cost lane as sufficient assurance; if missing-manifest failures recur, stop rather than paying for blind identical retries. Then consider `--fail-on-severity high`, a `gpt-5.6-terra --effort high` low-risk lane, and a separate scheduled or manually dispatched deep repository scan.
