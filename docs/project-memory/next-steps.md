...
- Evidence retry PR #379 passed every free aggregate but failed closed when the external autonomous review request failed after consuming its permanent exact-head claim. One same-run retry was deterministically denied and the PR closed unmerged. Issue #380 and the implemented learning artifact require the delivery skill to inspect that claim before rerunning, preserve the one-paid-call boundary, and prohibit no-op retry heads.
- Deliver branch `codex/autonomous-review-rerun-guidance` through terminal evidence. Then complete the issue #380 artifact with the exact fixed merge and retry the public-safe Phase 2 evidence PR while keeping the phase `in_progress`. Only a later protected ledger PR may mark Phase 2 `accepted` after the evidence PR itself has terminal evidence. Then begin Phase 3. Use proportional local validation and one deliberate push when possible; keep task-evaluation aliases fail closed until Phase 4 and do not weaken or bypass any verifier or gate.
