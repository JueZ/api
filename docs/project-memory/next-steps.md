...
- Finish Phase 3 on branch `codex/agent-learning-phase-3-triage`: validate deterministic conversion, rollout/backfill, idempotency, disposition, injection, secret-exclusion, and closure tests; update the trusted repair-workflow hash; then deliver one protected `[skip deploy]` PR and exercise an exact-range dry run plus one idempotent live candidate/link cycle. Keep task-evaluation aliases fail closed until Phase 4 and do not weaken or add a branch-protection context.
