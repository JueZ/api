...
- Deliver the current identical-compare regression capsule. Keep every controller/protected-main identity invariant, accept omitted `head_commit` only for the fully exact identical case, and retain strict head binding for all non-identical comparisons.
- After that capsule has terminal delivery/runtime evidence, retry the public-safe Phase 2 evidence PR while keeping the phase `in_progress`. Only a later protected ledger PR may mark Phase 2 `accepted` after the evidence PR itself has terminal evidence. Then begin Phase 3. Keep task-evaluation aliases fail closed until Phase 4; do not weaken or bypass any verifier or gate.
