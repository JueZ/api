...
- Bootstrap PR #274 repaired the trusted reviewer. Focused runtime/bootstrap/settings PR #275 superseded exhausted PRs #272/#273, then exhausted its own two high-risk review attempts; a fresh successor carries the review-required exact-run provenance repair.
...
4. Run the focused repair through exact-head PR CI, Policy Check, CodeQL, and the trusted merge controller. Require the deployment workflow to reject a source SHA once it is no longer current `main` at an Azure mutation boundary, and require every downstream dispatch to validate one correlated pinned run's Actions metadata and matching provenance. Production rollback must download only the bundle preserved by its exact accepted production run.
