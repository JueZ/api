...
- Phase 2 implementation PR #349 merged as `9310c94f97541e57f83b186af2cacf989d6f5330` and completed staged delivery/runtime verification, but the phase remains `in_progress`. Evidence/verifier PRs #350, #352, #354, #358, and #362 closed unmerged after their repair limits. Issues #351, #355, #356, #359, #360, and #361 record the required durable follow-up.
- Ledger PR #363 merged as `23f23f0751c5c3ac1aa6901b88b8e2aa8d3e0570` and completed exact-main CI, test/production delivery, smoke, telemetry, release-ledger, and fresh runtime-truth verification.
- Monitor the library-only successor from `codex/agent-learning-trusted-verifier-v4`. It preserves tokenless PR CI, fixed candidate paths, exact implementation identity, complete latest histories, reviewed workflow bytes, server-associated ledger artifacts/digests, fixed runtime hosts, disabled redirects, bounded reads, and the existing merge/delivery controls.
- The successor now rejects every self-recorded open-PR commit identity in changed governance ledgers, binds checkout/option to runtime `GITHUB_WORKFLOW_SHA`, authenticates that SHA as stable protected-main history, and re-fetches authoritative check/status/workflow histories before final evaluation. Exact PR and commit identities must be archived later from authenticated evidence. Do not mark Phase 2 accepted or wire credentials in this library PR.
- After that library merges, use a separate controller-wiring PR and then an evidence-only PR to exercise it, add verified artifacts for all six open candidates, and record final acceptance. Keep task-evaluation aliases fail closed until Phase 4, add no new protected context, and do not begin Phase 3 or paid/general task execution first. A merged PR alone remains insufficient runtime or live-configuration evidence.
