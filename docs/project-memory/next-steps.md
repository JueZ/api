...
- PR #386 merged the historical-lineage regression as `a56a932393a885976ad85f56df6cf3ba0f142e1b`, but Main Delivery `31329669160` failed before main CI on the superseded review-artifact consumer contract. Deliver issue #387 on branch `codex/fix-model-free-delivery-artifact`, then require terminal main CI, Deploy Test, Promote Production, smoke, telemetry, ledger, and runtime evidence. Only afterward retry the public-safe Phase 2 evidence PR while keeping the phase `in_progress`. A later protected ledger PR may mark Phase 2 `accepted`; then begin Phase 3. Keep task-evaluation aliases fail closed until Phase 4 and do not weaken or add a branch-protection context.
