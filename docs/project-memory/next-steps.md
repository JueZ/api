...
4. Run the focused repair through exact-head PR CI, Policy Check, CodeQL, and the trusted merge controller. Require the deployment workflow to reject a source SHA once it is no longer current `main` at an Azure mutation boundary, and require every downstream dispatch to resolve to one correlated pinned run with matching provenance.
