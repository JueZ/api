...
7. `CODEX_SECURITY_API_KEY` is configured. First require the Ubuntu 22.04 sandbox write probe and one Luna/high canary to produce a sealed manifest, complete coverage, valid findings, and SARIF. If either fails, stop before blind paid retries and re-evaluate the upstream CLI/runtime compatibility. After a successful canary, review approximately 10–20 advisory scans and measure estimated cost, runtime, completeness, deferred surfaces, false positives, and result-sealing reliability. Compare representative scans with Sol/xhigh before treating the lower-cost lane as sufficient assurance; then consider `--fail-on-severity high`, a `gpt-5.6-terra --effort high` low-risk lane, and a separate scheduled or manually dispatched deep repository scan.
