...
- Live readiness is now `AUTONOMOUS_REVIEW_CAPACITY_READY=true`. Make one deliberate protected push of `codex/autonomous-review-capacity-gate` and deliver the combined rerun-guidance/circuit-breaker/actor-rule-correction capsule through terminal evidence without a same-head rerun after claim consumption. Afterwards use one protected PR both to verify the three artifacts and register public-safe Phase 2 evidence, followed by one post-evidence Phase 2 acceptance-ledger PR before starting Phase 3. Do not inspect or replace the shared key, create a no-op head, or weaken any verifier or gate; keep task-evaluation aliases fail closed until Phase 4.
