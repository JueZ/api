...
- Phase 2 implementation PR #349 merged as `9310c94f97541e57f83b186af2cacf989d6f5330` and completed staged delivery/runtime verification, but the phase remains `in_progress`. Evidence/verifier PRs #350, #352, #354, #358, #362, and #364 closed unmerged. Issues #351, #355, #356, #359, #360, #361, #365, and #368 record the required durable follow-up.
- Ledger PR #366 merged as `98d99ef0743c680e65d8a7786fb98055068070b4` and completed exact-head, main-delivery, test/production smoke, telemetry, ledger, and fresh runtime-truth evidence. Deliver the protected trusted-primitives module and deterministic tests as the current independently useful PR, targeting at most 140,000 diff bytes. Follow with a separate Phase 2 evidence schema/collector/orchestrator PR that imports those merged primitives and stays under the same target.
- Preserve tokenless PR CI, fixed candidate paths and runtime hosts, disabled redirects, bounded reads, artifact digests, reviewed workflow bytes, exact implementation/candidate/controller identity, complete histories, stable final re-fetches, and every existing merge/delivery control. Do not raise the review cost ceiling, omit executable tests, mark Phase 2 accepted, or wire credentials in either library slice.
- After both library slices merge, use a separate controller-wiring PR and then an evidence-only PR to exercise it, add verified artifacts for all open candidates, and record final acceptance. Keep task-evaluation aliases fail closed until Phase 4, add no new protected context, and do not begin Phase 3 or paid/general task execution first. A merged PR alone remains insufficient runtime or live-configuration evidence.
