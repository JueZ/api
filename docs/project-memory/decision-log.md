...
- Decision: Cap the complete capsule at 200,000 bytes. After one durable exact-head claim, make one OpenAI input-token count request using the same model, reasoning, schema, and input as generation. Permit at most one generation with up to 3,000 output tokens only when the exact-input plus maximum-output calculation is no more than `$0.31`; disable SDK retries and revalidate free gates and claim ownership before both external-request boundaries.
...
- Status: The availability repair succeeded, but review `30693386513` correctly found that mixed high-risk documentation was still omitted. The scoped repair now includes every classified high-risk document without changing the 3,000-token or `$0.31` caps. Fresh free gates, one new-head review, merge, and test-only acceptance remain pending. Production is unauthorized.
