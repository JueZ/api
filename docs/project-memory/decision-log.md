...
- Supply-chain boundary: Fetch only the fixed scanner manifest and lockfile from the immutable same-repository PR head before checkout, validate the exact `0.1.3` dependency plus every registry integrity entry, and install with `npm ci --ignore-scripts`. Do not resolve a fresh transitive graph during a credentialed run.
- Credential boundary: Export knowledge files from the trusted base SHA into a private temporary directory and reject symlinks, submodules, and other non-regular entries. The untrusted PR head remains the code being scanned but cannot redirect a knowledge-base path to runner or credential data.
...
- Decision: During the low-spend integration canary, validate sandbox writing, official sealed-example SARIF export, and the immutable target with no model call. Then use `gpt-5.6-luna` with `low` effort and the operator-selected `$0.10` estimated stop threshold. `low` is the least effort accepted by Luna even though CLI 0.1.3 locally accepts `minimal`. Provide trusted-base agent/security/architecture/current-state knowledge, export sealed results to exact-head SARIF when available, and retain private result artifacts for seven days without automated PR comments.
...
- Rationale: The first authorized Sol/xhigh run reached an estimated `$1.322956` but failed to author the required scan manifest. After the Ubuntu 22.04 repair, Luna/high run `30745450036` proved workspace writing by creating regular hardening artifacts, then stopped at `$3.0123581` because the former `$3` threshold was exhausted before canonical finalization. The operator capped further setup calls at `$0.10`; that probe can test authentication and cost enforcement but cannot guarantee a sealed repository verdict.
