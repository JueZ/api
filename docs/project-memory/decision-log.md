...
- Decision: Treat the authenticated successful Actions job and ledger-publication steps plus exact-run artifact metadata, downloaded bytes, GitHub SHA-256 digest, and parsed ledger environment/origin as the run-bound deployment attestation. Derive live `/health` only from that digest-bound origin. GitHub exposes no immutable deployment-to-job field, so deployment status `log_url` is corroboration only; authenticated deployment/status records cross-check the expected environment and origin. Require the test and production attested origins to differ and require live health to agree on environment, run, and merge SHA. For `pull_request_target`, validate the Actions REST source head/branch separately from the authenticated PR base/head and custom review-check external identity.
- Root cause: Rejected PR #350 head `b928577c9a88218dcdacc7293f6264196837a80e` downloaded exact ledgers but still selected the live runtime origin from ledger content, so a false ledger could alias test and production. Review runs `31300174096` and `31300664072` on PR #352 then exposed that a status URL was not immutable job provenance and that authenticated artifact metadata/content still needed a cryptographic edge to the exact bytes selecting the runtime origin.
