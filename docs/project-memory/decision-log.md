...
- Decision: Required CI, Policy Check, immutable release construction, deployment smoke/telemetry/ledger, and repair triage invoke fixed local binaries or script files instead of package scripts. Install dependencies with lifecycle scripts disabled in those workflows. Make the trusted controller allow only exact lifecycle-disabled install, lockfile-only install, and audit commands; reject every other npm, npx, Yarn, pnpm, Corepack, Bun, or Node `--run` command in workflow `run` blocks.
