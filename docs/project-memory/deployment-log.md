...
## 2026-07-31 — Enterprise hardening passed engineering gates; credential review activated a deployment hold

- PR #285 exact head `6f3d7652100ec6105576206f6d70b7451a50ede3` passed CI run `30654563388`, Policy Check run `30654563541`, CodeQL, and immutable release-artifact checks. First independent high-risk review run `30654561912` rejected the change because branch-protection rendering did not pin the expected GitHub App, service-token preflight accepted an incomplete claim contract, and repository delivery remained available while exposed local credentials awaited external rotation.
- The bounded repair makes `.github/autonomous-policy.yml` authoritative for exact check/app-ID/source pairs, renders app-bound `checks` without legacy contexts, and adds fail-closed live verification. Read-only verification proves all 23 current `main` checks use GitHub Actions App ID `15368` with strict status checks, admin enforcement, linear history, conversation resolution, and force-push/deletion protection intact. The controller additionally resolves Actions details URLs and validates exact workflow ID/path, event, attempt, repository, PR/base/head, run/job identity, and SHA; the controller check must match its recorded check-run ID.
- Smoke-token preflight now validates exact v1/v2 tenant, client, tenant-bound issuer, configured API audience, application-only evidence, nonempty subject, and an exact unique least-privilege role set. Missing or extra roles fail before export; delegated or malformed claims are rejected without logging claims or token material.
- Exact static first steps in the shared deployment, private-storage migration, Bring service-token canary, and Azure OIDC diagnostic workflows now fail before checkout, token minting, OIDC, secrets, or Azure access. Live `test`/`production` environments were restricted to protected branches and the repo OIDC subject changed from default to exact `repo,context,job_workflow_ref`; this intentionally invalidates old Azure FICs. Bring canary, Codex Main Delivery, Deploy Test, Promote Production, Rollback Production, Private Storage Migration, and Verify Azure OIDC are `disabled_manually`; production enablement is false. GitHub rejected a non-enforcing required-workflow ruleset on this user-owned repository; because same-repository workflows share App ID `15368`, repository Actions and native auto-merge were disabled to prevent secret/check spoofing. The hold is active-only because no independent security approver exists, and security-control paths cannot auto-merge. No test or production deployment ran. External rotation, out-of-band trust bootstrap, replacement workflow-bound FICs, and a fresh test-first deployment remain required.
