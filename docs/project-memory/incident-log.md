...
- Prevention: token minting now validates the exact version-specific identity and role set before exporting the token and reports only bounded missing or unexpected role names for a sole role mismatch. Never restore retired roles, broaden API authorization, or bypass authenticated smoke.
...
- Deployment containment: PR #285 independent review run `30654561912` correctly rejected repository-only ignore/permission controls while delivery remained available. The repair adds exact hard-coded no-bypass blocks before cloud access, expands parsed inventory across all workflow/composite YAML for OIDC, environment, login, and cloud-command capabilities, and disables Bring canary, main delivery, test, production, rollback, migration, and Azure OIDC diagnostic entrypoints while keeping production enablement false. Live deployment environments now accept protected branches only and the repository OIDC template binds `repo`, `context`, and `job_workflow_ref`, intentionally breaking old Azure FICs.
- Clearance correction: adversarial review proved that a JueZ comment cannot distinguish the exposed GitHub credential from a rotated/hardware-backed session. The GitHub-comment clearance implementation was removed; the checked-in incident schema is active-only and rejects `active=false` or local approval. Security-control paths are excluded from autonomous merge. GitHub also rejected a non-enforcing required-workflow ruleset for this user-owned repository; because all Actions checks share one App identity and feature workflows could read repository secrets, repository Actions and native auto-merge were suspended. Recovery requires external GitHub revocation followed by an independently verified out-of-band trust root and replacement workflow-bound Azure FICs. Migration also revalidates live main/hold immediately before login and before/after upload; API failure, rerun, or drift fails closed.
- Final adversarial follow-up: rename handling now classifies both GitHub `filename` and `previous_filename`; root npm manifests/lock, delivery helpers and transitive release schema are non-autonomous trust roots; repository shrinkwrap and `.npmrc` install overrides are rejected. The Bring token canary and Azure OIDC diagnostic have the same static first-step block as deployment/migration. The secret-safe live verifier also requires zero requested, waiting, pending, queued, or in-progress Actions runs, preventing a disabled-but-still-running workflow from being reported as contained.
