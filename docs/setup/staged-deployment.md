...
For the production WLH cutover, `prepare-production-private-storage.yml` calls the existing workflow-bound deployment identity in storage-preparation-only mode. It requires exact current-main CI and accepted Deploy Test provenance, previews a shared-Bicep storage-only change set, permits only the fixed approved source/target/blob/digest tuple, refuses overwrite, verifies the copied bytes and storage policy, and proves the production Function still reports the prior accepted runtime identity. Keep this workflow disabled outside its bounded preparation window.
