...
In a billing-enabled Google Cloud project, enable **Weather API** in Google Maps Platform and create an API key. Restrict the key to the Weather API and apply appropriate application restrictions. Store it as the GitHub environment secret `GOOGLE_WEATHER_API_KEY`; never put it in a variable, caller argument, source file, or log. Set the environment variable `WEATHER_ENABLED=true`. Delivery stores the secret in Azure Key Vault and configures the Function App with a Key Vault reference. With `WEATHER_ENABLED=false`, no key is required and calls return the standard provider-disabled error.
