...
GitHub Actions obtains a GitHub OIDC assertion and exchanges it for a short-lived Entra access token. No static API bearer token or service client secret is stored. `scripts/configure-entra-service-oauth.sh` is a test-only helper locked to the complete `JueZ/api` / `test` / `JueZ/api/.github/workflows/deploy-environment.yml@refs/heads/main` / `JueZ API Catalogue Service Test` tuple. Caller overrides may assert the first three exact values but cannot select another repository, environment, workflow/ref, or service app. The helper requires one unique existing approved service app and its existing named test FIC; it never creates another app or trust route. It rejects destructive Bring roles.
