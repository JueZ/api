...
- The exhaustive path checkpoints normalized comments, global comment deduplication, its sort-aware traversal frontier, and a deterministic multi-sort discovery plan (`requested`, `old`, `new`, `controversial`, `top`, `confidence`, `qa`, with duplicates removed) in a private Blob after each bounded invocation. All supported sorts are retained because Reddit's bounded, differently ordered listings can expose distinct comments or MoreChildren work; `qa` runs last to limit the common-case cost of its heavy overlap. Opaque signed cursors reference the snapshot and page offset; callers never carry Reddit child-ID queues.
...
- A high configurable snapshot resource cap protects Function memory and storage. Reaching it is a terminal `resource_limited` snapshot with incomplete coverage, so it cannot create an infinite cursor.
