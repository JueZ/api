...
- The exhaustive path checkpoints normalized comments, global comment deduplication, its sort-aware traversal frontier, and a deterministic multi-sort discovery plan in a private Blob after each bounded invocation. Opaque signed cursors reference the snapshot and page offset; callers never carry Reddit child-ID queues.
