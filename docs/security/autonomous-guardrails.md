...
Root package manifests and lockfiles, lint/type/build configuration, every executable application path, and every repository script are high risk. Required CI, policy, release-construction, deployment-smoke, telemetry, and ledger steps invoke their pinned tools or script files directly; they never dispatch a command through a PR-controlled `npm run` or `npm test` definition. The trusted controller audits every workflow and rejects repository-package-script indirection before review or merge. Dependency installation in these paths uses `--ignore-scripts`. A change that removes or remaps a package script therefore cannot turn a required check or release step into a successful no-op, and executable code cannot obtain deterministic low-risk approval.
