...
Phase-program acceptance evidence has a second trusted boundary immediately before merge. The protected-main exact-head job runs the fixed verifier only after exact-head review evidence exists and before the complete-rollup merge gate. Repository credentials remain confined to that trusted checkout; candidate files are fetched as bounded data and are never executed. Every candidate is bound to the exact repository, PR, head/base, changed-file history, controller run/workflow SHA, review artifact, and a stable final re-fetch. Full live GitHub, deployment-workflow/job, release-ledger, and runtime verification is activated only by a Phase 2 evidence or acceptance change. Successful output contains only a sanitized status and immutable public identities; it is uploaded under an exact-head name, while a missing output or verifier error fails closed. It cannot substitute for the existing review, rollup, deployment, smoke, telemetry, or ledger gates.
