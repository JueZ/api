...
Learning records contain only public-safe structured evidence. Validation requires normalized fingerprints and repository-contained existing paths, exact lowercase full commits for verified counterfactuals, and an owned current exception for a waiver or no-artifact disposition. A registered trusted scorer reads exact historical Git objects as data and proves the broken/fixed invariant without executing historical code; required CI uses read-only authenticated GitHub metadata to bind the declared merged PR, base SHA, and merge SHA. When supported program-acceptance evidence changes, the same job authenticates each aggregate check and immutable workflow run, downloads the recorded ledger artifact ID directly, verifies the received archive bytes against GitHub's authenticated SHA-256 digest, accepts only the canonical ledger file, and rechecks live runtime identity from the digest-bound origin. The authenticated successful job, exact ledger-publication steps, artifact run metadata/digest, and ledger environment/origin are the run-bound attestation. Because GitHub exposes no immutable deployment-to-job field, deployment/status metadata cross-checks the environment and origin while its `log_url` remains corroboration only. The verifier requires the expected GitHub Actions bot identity, distinct `test` and `production` attested origins, and live `/health` environment/run/SHA agreement. It also distinguishes authenticated Actions REST source-head metadata for `pull_request_target` from the runner's base-context SHA and separately binds the PR base/head and custom review-check external identity. Missing authentication or any check, run, job, deployment, artifact, digest, ledger, smoke, telemetry, environment, URL, or runtime mismatch fails closed; self-authored evidence fields never establish acceptance by themselves. Validation rejects secret-shaped values, credential-bearing URLs, raw environment dumps, and private provider content. A waiver is never a passing proof. Fixed-path learning validation and deterministic index checking run within the existing architecture job and `CI complete`; no additional protected context or paid general agent evaluation is introduced.
