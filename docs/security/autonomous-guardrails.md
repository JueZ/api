...
- Immutable release construction may run in parallel with independent validation after the standalone install job succeeds. It remains an explicit `CI complete` dependency, and deployment accepts its artifact only when that exact main CI run completed successfully; parallel scheduling does not make any validation optional or allow an artifact from a failed run to deploy.
...
Environment deployment may be omitted after successful exact-main CI only when protected-main code authenticates the merged PR, retrieves its complete paginated file list, matches the exact GitHub changed-file count, and proves every current and previous rename path belongs to the fixed runtime-neutral allowlist. Besides reviewed Markdown and scoped instructions, that allowlist may include only the dedicated agent-task definitions, non-shipped agent-learning/task-evaluation controllers, and their scoped deterministic tests. Those paths remain high risk and keep every PR aggregate, fixed-path validation, immutable release construction, exact-main CI, security scan, provenance, and complete-rollup defense. A missing PR, malformed or duplicate path, path traversal, count mismatch, mixed change, or any application, workflow, policy, package, contract, infrastructure, runtime, or other script path remains deployment-required. Explicit operator skip markers remain supported.
