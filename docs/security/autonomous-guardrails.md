...
Program acceptance evidence has two explicit trust levels. Tokenless pull-request CI invokes only the offline fixed-path registration/schema check. Authenticated collection runs after independent review from the `github.workflow_sha` protected-main checkout with persisted credentials disabled and read-only `actions`, `checks`, `contents`, `deployments`, and `pull-requests` permissions. It binds the repository, open PR, exact head/base, first-attempt controller run, newly consumed review claim, complete latest check/status rollups, canonical workflow histories, implementation merge, deployment histories/jobs/statuses, and immutable release-ledger artifacts. Phase 2 additionally requires the protected verifier's fixed PR #349 implementation identity—exact base, source branch, reviewed head, and merge—before live evidence is queried. Candidate-controlled files are limited to the authoritative program and registered Phase 2 evidence paths, fetched at exact SHAs and parsed only as size-bounded strict data; they cannot select commands, repository paths, artifact endpoints, URLs, or credentials.

Raw ledger archive bytes must match both authenticated GitHub digest metadata and the public evidence digest before a single canonical ledger entry is accepted. A later pending, failed, cancelled, or wrong-App check/run/status cannot be hidden behind an earlier success. A bot-authored historical `inactive` status is treated only as supersession, never as passing proof: current protected main is read before and after verification, and each environment's canonical latest deployment/status plus fixed-origin live health must match that unchanged exact main SHA and deployment run. Credentials, aliases, ports, non-root paths, query/fragment data, and redirects are rejected; response sizes, page counts, and deadlines are bounded. Missing, expired, malformed, stale, skipped-when-required, cancelled, non-success, wrong-App, wrong-head, wrong-environment, digest-mismatched, redirected, or unavailable evidence fails closed and prevents publication of `Autonomous review complete`. The trusted verifier emits only sanitized identifiers and conclusions and never rewrites program, instructions, skills, policies, scorers, workflows, or project memory.
