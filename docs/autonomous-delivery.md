...
Learning validation runs by fixed script paths inside `architecture and agent validation`, so it remains mandatory behind `CI complete` without adding a brittle protected context. When a supported program-evidence record changes, that same entry point uses the job-scoped read-only GitHub token to bind exact aggregate checks and immutable workflow runs, download the unique exact-run release-ledger artifact, validate public/authenticated smoke and telemetry results, and recheck live runtime identity. The authenticated workflow job and artifact metadata/content form the run-bound deployment attestation; a deployment status `log_url` is only corroboration because GitHub exposes no immutable deployment-to-job field. Authenticated GitHub deployment/status records instead bind each ledger and live `/health` request to the expected `test` or `production` environment and public origin; the origins must be distinct, and health must report the attested run and merge. For `pull_request_target`, the verifier treats the Actions REST run `head_sha`/`head_branch` as source-head metadata, distinct from the runner's base-context `GITHUB_SHA`, and separately authenticates the PR's exact base and head plus the review check's run-bound external identity. It fails closed on missing authentication, stale or expired artifacts, mismatched SHAs, workflow paths, run attempts, apps, jobs, environments, URLs, or runtime state; editable identifiers in the evidence file are not proof. `AGENTS.md`, repository skills, learning records, task definitions, and trusted scorer/controller paths are independently reviewed high-risk agent-governance changes. Failure evidence can create a candidate, but cannot directly rewrite those controls; implementation and any waiver require a normal protected PR. A waiver does not count as verified proof.
