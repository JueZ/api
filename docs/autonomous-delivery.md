...
6. binds that marker to repository, PR, exact head, trusted controller workflow, and workflow run; re-reads it after creation and again at the generation boundary; and permits the OpenAI request only while exactly that canonical marker and every free check still pass;
...
8. makes at most one independent structured model-generation request with `store=false`, a 3,500-token output cap, explicit final-JSON capacity reservation, and no SDK retries;
...
The live API path is accepted only from the exact `Codex Auto-Merge` GitHub Actions run. The standalone claim command has been removed, and the marker ID, canonical external identity, details URL, App identity, status, and conclusion must all match before the OpenAI request. The model capsule contains the complete contextual diff for every changed non-documentation path, including executable policy helpers that are not themselves classified as high risk, and every classifier-matched high-risk documentation path, plus metadata for every changed file. Ordinary mixed `docs/` diffs may be omitted, while documentation-only high-risk PRs retain every changed documentation diff. Capsule completeness is checked against GitHub's authoritative changed-file list and deterministic risk classification. There is no repository-configured review-input byte ceiling, exact-token preflight request, or per-head dollar ceiling. If GitHub does not supply every required changed path, a free gate or marker changes before generation, or the single model response is unavailable or malformed, the controller fails closed and the marker remains consumed. Retrying requires a genuinely repaired new commit and a fresh full set of free gates.
