...
9. from the same protected-main review job, authenticates the candidate, controller, and review identities and runs full live Phase 2 evidence verification only when the candidate changes the Phase 2 acceptance record or evidence file; ordinary candidates return a fail-closed sanitized `not_applicable` result after the same identity and final-head stability checks;
10. uploads that sanitized verification result and publishes `Autonomous review complete` successfully only when both the exact-head independent review and protected program-evidence verification succeeded;
11. rechecks open/current/non-behind PR state and the complete latest exact-head check-run and legacy-status rollup; aggregate `unstable` is accepted only when its sole pending cause is the current trusted `merge exact PR head` job;
...
The live API path is accepted only from the exact `Codex Auto-Merge` GitHub Actions run. The standalone claim command has been removed, and the marker ID, canonical external identity, details URL, App identity, status, and conclusion must all match before either OpenAI request. After deterministic risk classification, high-risk review requires the non-secret repository variable `AUTONOMOUS_REVIEW_CAPACITY_READY` to equal `true`; missing, false, or malformed capacity writes sanitized rejection evidence and fails the existing review aggregate before source-diff collection, permanent claim creation, token counting, or model generation. Low-risk changes remain eligible for deterministic approval without model capacity. A trusted sanitized `credit_balance_exhausted` result may move the variable to `false`; it may return to `true` when usable capacity is known to be available. The value records capacity state, not approval or authorization. The shared API key is never inspected, replaced, or revealed as recovery.

The bounded model capsule contains the complete contextual diff for every changed non-documentation path, including executable policy helpers that are not themselves classified as high risk, and every classifier-matched high-risk documentation path, plus metadata for every changed file. Ordinary mixed `docs/` diffs may be omitted, while documentation-only high-risk PRs retain every changed documentation diff. Capsule completeness is checked against GitHub's authoritative changed-file list and deterministic risk classification. A hard 200,000-byte capsule limit rejects oversized reviews without silent truncation; the OpenAI input-token endpoint then counts the exact structured request, and the model-generation maximum—not an approximate byte or line count—must remain under the configured cost ceiling. If a free gate or marker changes after creation, before counting, or before generation, the controller fails closed and the marker remains consumed. Before requesting a workflow rerun, Codex must inspect the exact-head rollup for the canonical completed neutral paid-call claim. An existing claim forbids a same-head autonomous-review rerun; recovery requires a genuinely repaired substantive new commit and a fresh full set of free gates. No-op or metadata-only retry commits are not recovery. If no substantive correction exists for an external failure, delivery stops blocked with a learning disposition instead of consuming time on a deterministic denial.
...
Protected `main` intentionally requires only four stable aggregate contexts from GitHub Actions: `CI complete`, `Policy complete`, `CodeQL complete`, and `Autonomous review complete`. `CI complete` and `Policy complete` use explicit `needs` lists plus `if: always()` and reject every non-success dependency result. `CodeQL complete` waits for the complete analysis matrix and likewise rejects every non-success result. `Autonomous review complete` succeeds only when the exact-head review job—including applicable protected program-evidence verification—succeeds. The PR-inapplicable main-only release attestation remains required when applicable but is not included in the PR `CI complete` dependency set.
