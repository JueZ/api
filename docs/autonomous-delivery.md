...
When a failed promotion leaves uncertain configuration, `recover-production` with an exact `failedRunId` explicitly reconciles current Bicep and runtime policy through the same trusted reusable workflow and production lock. It requires the retained accepted baseline, original recovery bundle, and attributed failed mutation; newer or ambiguous state blocks writes. Full smoke, authenticated smoke, telemetry, and a new recovery ledger must pass before the restored bundle becomes an accepted baseline. Normal full delivery then completes the current requested candidate. This mode adds no federation identity or alternate deployment controller.
...
Before production writes, the workflow downloads and verifies the previous accepted bundle and ledger against the installed versioned Function package and exact frontend inventory. Health is corroboration, not the sole identity source. Package-only rollback checks the smoke identity against installed authentication settings read from the Azure control plane; promotion and explicit configuration reconciliation check the intended configuration. A missing or expired recovery bundle blocks promotion. Long-term artifact retention remains a separate operational requirement.
