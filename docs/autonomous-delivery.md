...
For applicable Phase 2 transitions, the trusted verifier first requires the immutable implementation identity for PR #349: exact base commit, source branch, reviewed head, and merge commit. It then re-queries that PR, aggregate check runs, first-attempt workflow runs, deployment records/statuses/jobs, and named release-ledger artifacts. This prevents an unrelated successful delivery from satisfying Phase 2 acceptance. The raw artifact archive must match authenticated GitHub digest metadata and the candidate's recorded digest before its single canonical JSON entry is parsed. Runtime truth is fetched only from the exact allowlisted test and production Function origins with redirects disabled and bounded JSON responses. A missing, expired, malformed, mismatched, redirected, aliased, unavailable, cancelled, skipped, or non-success record blocks `Autonomous review complete`; a waiver or local pass cannot substitute for that evidence.
