...
Learning validation runs by fixed script paths inside `architecture and agent validation`, so it remains mandatory behind `CI complete` without adding a brittle protected context. When a supported program-evidence record changes, that same entry point uses the job-scoped read-only GitHub token to bind exact aggregate checks and immutable workflow runs, download the exact release-ledger artifacts, validate public/authenticated smoke and telemetry results, and recheck live runtime identity. It fails closed on missing authentication, stale or expired artifacts, mismatched SHAs, workflow paths, run attempts, apps, or runtime state; editable identifiers in the evidence file are not proof. `AGENTS.md`, repository skills, learning records, task definitions, and trusted scorer/controller paths are independently reviewed high-risk agent-governance changes. Failure evidence can create a candidate, but cannot directly rewrite those controls; implementation and any waiver require a normal protected PR. A waiver does not count as verified proof.
