...
6. binds that marker to repository, PR, exact head, trusted controller workflow, and workflow run; re-reads it after creation and again at the API boundary; and permits the single bounded request only while exactly that canonical marker and every free check still pass;
...
The live API path is accepted only from the exact `Codex Auto-Merge` GitHub Actions run. The standalone claim command has been removed, and the marker ID, canonical external identity, details URL, App identity, status, and conclusion must all match before the request. The bounded model capsule contains exact zero-context changes for executable/governance high-risk paths plus metadata for every changed file; mixed documentation diffs are omitted from the paid capsule, while documentation-only high-risk PRs retain their exact documentation changes. Capsule completeness is checked against deterministic path classification, and the serialized request—not an approximate line count—must remain under the configured cost ceiling. If a free gate or marker changes in the narrow interval after creation but before the API boundary, the controller fails closed and the marker remains consumed. Retrying a paid review requires a genuinely repaired new commit and a fresh full set of free gates.
