...
9. after an approving independent review, runs the protected-main program-evidence verifier with read-only GitHub access; the verifier treats only fixed allowlisted candidate program/evidence files as inert data and emits a sanitized result;
10. publishes `Autonomous review complete` for that exact SHA only when review and any applicable trusted program-evidence verification both succeed;
...
Authenticated agent-learning acceptance is never performed by candidate code. The trusted program-evidence job checks out `github.workflow_sha` with persisted credentials disabled, installs only that protected controller's lockfile, and receives read-only `actions`, `checks`, `contents`, `deployments`, and `pull-requests` access. It downloads the sanitized review decision from the same controller run, proves the exact reviewed head and permanent claim identity, and reads only the fixed authoritative program and registered Phase 2 evidence paths from GitHub's Contents API at exact SHAs. The candidate cannot select a file, URL, command, artifact endpoint, repository, or host.

For applicable Phase 2 transitions, the trusted verifier first requires the immutable implementation identity for PR #349: exact base commit, source branch, reviewed head, and merge commit. It then re-queries that PR; the complete paginated exact-head check and legacy-status rollup; canonical latest applicable aggregate and delivery workflow histories; deployment histories, statuses, and jobs; and named release-ledger artifacts. This prevents an unrelated delivery or an earlier success superseded by a later pending, cancelled, or failed record from satisfying Phase 2 acceptance. Historical deployment `inactive` is accepted only as a bot-authored supersession of the exact successful job with no intervening failure; the verifier separately reads protected main before and after fixed-host runtime checks and requires each environment's latest deployment, latest status, `/health` commit/source ref, and deployment run to match that unchanged exact main SHA. The raw artifact archive must match authenticated GitHub digest metadata and the candidate's recorded digest before its single canonical JSON entry is parsed. A missing, expired, malformed, mismatched, redirected, aliased, unavailable, cancelled, skipped-when-required, or non-success record blocks `Autonomous review complete`; a waiver or local pass cannot substitute for that evidence.
