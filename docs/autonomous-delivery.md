...
An explicit manual `mode=full` dispatch runs the complete protected delivery even when that SHA is already accepted. This supports configuration-only changes and delivery qualification without manufacturing a source change. It must target exact current protected main and retains baseline, identity, test, production and recovery gates. Neutral pushes still skip unnecessary deployment. The production configuration comparison independently selects full reconciliation or the qualified application-only path.
...
The application-only decision requires identical accepted inputs and fresh configuration, plus complete whole-template what-if coverage. Missing managed resources, nested children, analysis diagnostics, unresolved potential changes, different secrets/configuration/toolchain, or drift select full reconciliation. An untouched resource outside the accepted inventory may appear as `Ignore`; it cannot count toward managed coverage, carry a delta, cross the resource-group boundary, or duplicate another ID. Only independently checked reference substitutions, exact provider-default response differences, and the accepted-to-candidate retention transition may remain. Schema 2 captures the relevant budget, vault, telemetry, storage and web settings; older records require full reconciliation. Default exceptions match exact kinds and values, with a currently active budget interval and live equality to accepted state. They cannot accept an arbitrary modification or learn an out-of-band change as harmless noise. Configuration is recaptured before updating retention; application deployment, complete runtime policy, smoke, telemetry, release identity and recovery checks remain required. A verified Y1/Dynamic Linux Node plan bounds the inapplicable elastic/framework fields. App-only acceptance cannot refresh drift. If a full deployment cannot capture optional comparison evidence, it publishes no pointer and the next deployment stays full.
