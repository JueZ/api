...
function buildIndex(rows: any[]): CategoryIndex { const byId = new Map<string, WlhCategory>(); const childrenByParentId = new Map<string, WlhCategory[]>(); const sourceById = new Map<string, any>();
  for (const row of rows) { const id = String(row.id); sourceById.set(id,row); const c: WlhCategory = { id, label: String(row.label ?? ''), path: String(row.path ?? ''), depth: Number(row.depth ?? 0), parentId: row.parent_id != null ? String(row.parent_id) : undefined, hitCount: typeof row.hits === 'number' ? row.hits : undefined, hasChildren: Array.isArray(row.children) && row.children.length > 0, url: typeof row.url === 'string' ? row.url : undefined }; byId.set(id, c); }
  for (const [id,row] of sourceById) { const ids = Array.isArray(row.children)?row.children.map((x:any)=>String(x)):[]; childrenByParentId.set(id, ids.map((cid:string)=>byId.get(cid)).filter(Boolean) as WlhCategory[]); }
