...
export class WlhService {
  ...
  async search(body: unknown) {
    ...
    return { source: 'wlh', fetchedAt: new Date().toISOString(), sourceUrl: res.url, rowsFound: res.data?.props?.pageProps?.searchResult?.numFound ?? null, rowsReturned: rows.length, filteredRowsReturned: filtered.length, category: cat, results: filtered.map(({ _body, ...x }: any) => x) };
  }
  ...
  async offer(adId: string) {
    ...
    const locationBlock = d.advertAddressDetails ?? d.advertContactDetails ?? {};
    const p2p = d?.p2ppOptions; const paylivery = Boolean(firstAttr(attrs, 'PAYLIVERY') ?? firstAttr(attrs, 'p2penabled') ?? (p2p?.deliveryOptions?.length || p2p?.paymentOptions?.length));
    ...
    return { id: String(firstAttr(attrs, 'ADID') ?? d.id ?? adId), title: String(firstAttr(attrs, 'HEADING') ?? d.title ?? ''), description: htmlToText(String(firstAttr(attrs, 'DESCRIPTION') ?? firstAttr(attrs, 'BODY_DYN') ?? d.description ?? '')),
      priceAmount: parsePrice(firstAttr(attrs, 'PRICE/AMOUNT') ?? firstAttr(attrs, 'PRICE')), priceDisplay: String(firstAttr(attrs, 'PRICE_FOR_DISPLAY') ?? d?.price?.description ?? ''), location: String(firstAttr(attrs, 'LOCATION') ?? firstAttr(attrs, 'DISTRICT') ?? locationBlock.city ?? ''), postcode: String(firstAttr(attrs, 'POSTCODE') ?? locationBlock.postcode ?? ''), state: String(firstAttr(attrs, 'STATE') ?? locationBlock.state ?? ''),
      publishedAt: parseDate(firstAttr(attrs, 'PUBLISHED') ?? firstAttr(attrs, 'PUBLISHED_String')), changedAt: parseDate(firstAttr(attrs, 'CHANGED') ?? firstAttr(attrs, 'CHANGED_String')), status: d.status,
