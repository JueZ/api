...
export function normalizeSearchRequest(v: unknown): Required<Pick<WlhSearchRequest, 'categoryId' | 'rows' | 'page'>> & WlhSearchRequest {
  ...
  if (typeof b.categoryId !== 'string' || !b.categoryId.trim()) throw new WlhInputError('categoryId is required.', 'categoryId');
  ...
  if (b.delivery && (!Array.isArray(b.delivery) || b.delivery.some((d) => !deliveries.has(d)))) throw new WlhInputError('delivery is invalid.', 'delivery');
