...
export const DEFAULT_TASK_DIRECTORY = join(REPOSITORY_ROOT, 'evals/agent-tasks');
export const DEFAULT_RESULTS_DIRECTORY = join(REPOSITORY_ROOT, '.agent-eval-results');
...
export function validateTaskRepository(options = {}) {
  ...
  const taskDirectory = resolve(options.taskDirectory ?? join(repositoryRoot, 'evals/agent-tasks'));
  const errors = [];
