...
function parseArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (arg === '--confirm-paid-agent-eval') result.confirmPaid = true;
  ...
  if (result.adapterId === 'codex-cli' && !result.confirmPaid) {
    throw new Error('Real Codex execution is optional and requires --confirm-paid-agent-eval');
  }
  if (result.adapterId === 'fake-adapter' && result.confirmPaid)
    throw new Error('Paid confirmation is invalid for fake-adapter');
  return result;
}
