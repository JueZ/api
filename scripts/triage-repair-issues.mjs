...
export function parseRepairIssueBody(body = '') {
  ...
  const environment = lower.includes('production') || lower.includes('prod') ? 'prod' : lower.includes('test') ? 'test' : '';
  const productionFailure = /production|prod|promote-production|smoke|\/health|runtime|deploy/i.test(body);
  return { prNumbers: [...new Set(prNumbers)], workflowRunUrls: [...new Set(workflowRunUrls)], workflowRunIds: [...new Set(workflowRunIds)], commitShas: [...new Set(commitShas)], environment, productionFailure };
}

export function classifyRepairIssue(issue, parsed = parseRepairIssueBody(issue?.body || '')) {
  const labels = (issue?.labels || []).map((label) => typeof label === 'string' ? label : label.name).filter(Boolean).map((label) => label.toLowerCase());
  const text = `${issue?.title || ''}\n${issue?.body || ''}\n${labels.join(' ')}`.toLowerCase();
  if (/production|prod|promote-production|smoke|\/health|runtime/.test(text) || labels.includes('production-failure')) return 'production_failure';
...
export function hasDuplicateTriageComment(comments = [], issueNumber, decisionKind, reason = '') {
  ...
  return comments.some((comment) => String(comment.body || '').includes(marker) && (!reason || String(comment.body || '').includes(reason)));
}

function prEvidenceComplete(prStates = []) {
  return prStates.some((pr) => pr.merged === true || pr.state === 'MERGED') && prStates.some((pr) => pr.checksPassed === true || pr.statusCheckRollup?.some?.((check) => check.conclusion === 'SUCCESS'));
}
...
export function productionVerificationPassed(verification = {}, options = {}) {
  ...
  if (validationErrors.length > 0) return { ok: false, reason: `release ledger validation failed: ${validationErrors.join('; ')}` };
  if (verification.workflowConclusion && verification.workflowConclusion !== 'success') return { ok: false, reason: `Promote Production run conclusion is ${verification.workflowConclusion}` };
  if (ledger.environment !== 'prod') return { ok: false, reason: `release ledger environment is ${ledger.environment}` };
  if (ledger.smokeResults?.status !== 'passed') return { ok: false, reason: `runtime smoke status is ${ledger.smokeResults?.status || '<missing>'}` };
  if (ledger.authenticatedSmokeResults?.status !== 'passed') return { ok: false, reason: `authenticated smoke status is ${ledger.authenticatedSmokeResults?.status || '<missing>'}` };
  if (ledger.telemetryCheckResult?.status !== 'passed' && !(options.acceptBlockedTelemetry === true && ledger.telemetryCheckResult?.status === 'blocked_telemetry')) return { ok: false, reason: `telemetry status is ${ledger.telemetryCheckResult?.status || '<missing>'}` };
  if (verification.liveHealth?.deployedCommitSha && verification.liveHealth.deployedCommitSha !== ledger.deployedCommit) return { ok: false, reason: `live /health SHA ${verification.liveHealth.deployedCommitSha} does not match ledger ${ledger.deployedCommit}` };
...
export function decideRepairIssueAction(issue, evidence = {}) {
  ...
  if (issueType === 'pr_check_failure') {
    if (prEvidenceComplete(evidence.prStates || []) && !evidence.laterProductionFailure) return { action: 'close', decisionKind: 'resolved-pr-check', issueType, reason: 'referenced PR merged and CI/policy evidence succeeded with no later linked production failure' };
    return { action: 'comment', decisionKind: 'evidence-needed', issueType, reason: 'PR-check repair evidence is incomplete or a later production failure is linked' };
  }
  if (issueType === 'production_failure' || issueType === 'deployment_failure') {
    const verified = productionVerificationPassed(evidence.productionVerification || {}, { acceptBlockedTelemetry: evidence.acceptBlockedTelemetry });
    if (verified.ok) return { action: 'close', decisionKind: 'resolved-production-verification', issueType, reason: verified.reason };
    return { action: 'comment', decisionKind: 'production-evidence-needed', issueType, reason: verified.reason };
  }
  return { action: 'comment', decisionKind: 'unknown-evidence-needed', issueType, reason: 'codex-repair issue type is unknown; leaving open until resolution evidence is explicit' };
}
...
async function collectLatestProductionVerification({ repo, apiBaseUrl = '', requireLedger = true }) {
  const runs = JSON.parse(gh(['run', 'list', '--repo', repo, '--workflow', 'promote-production.yml', '--branch', 'main', '--status', 'success', '--limit', '20', '--json', 'databaseId,headSha,conclusion,status']));
  for (const run of runs) {
    ...
    try {
      ...
      return { workflowRunId: String(run.databaseId), workflowConclusion: run.conclusion, artifactName, ledger, validationErrors, liveHealth };
    } catch {
...
async function collectPrStates(parsed, repo) {
  ...
  for (const number of parsed.prNumbers) {
    try {
      const pr = JSON.parse(gh(['pr', 'view', String(number), '--repo', repo, '--json', 'number,state,merged,mergeCommit,statusCheckRollup,url']));
      pr.checksPassed = (pr.statusCheckRollup || []).length > 0 && (pr.statusCheckRollup || []).every((check) => check.conclusion === 'SUCCESS' || check.conclusion === 'SKIPPED');
      states.push(pr);
    } catch { /* best effort */ }
  }
  return states;
...
export async function runRepairTriage({ env = process.env } = {}) {
  ...
  const issues = JSON.parse(gh(['issue', 'list', '--repo', repo, '--label', 'codex-repair', '--state', 'open', '--json', 'number,title,body,url,labels']));
  ...
  for (const issue of issues) {
    ...
    const evidence = { parsed, issueType: classifyRepairIssue(issue, parsed), prStates, productionVerification, dryRun };
    ...
    if (!dryRun) {
      ...
      if (!hasDuplicateTriageComment(comments, issue.number, decision.decisionKind, decision.reason)) gh(['issue', 'comment', String(issue.number), '--repo', repo, '--body', body]);
      if (decision.action === 'close' && closeResolved) gh(['issue', 'close', String(issue.number), '--repo', repo, '--comment', `Closing after evidence-based Codex repair triage: ${decision.reason}.`]);
    }
