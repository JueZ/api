...
API_SCOPE_VALUES="${API_SCOPE_VALUES:-catalogue.read,reddit.read,youtube.read,wlh.read,weather.read,bring.read,bring.write,bring.complete,bring.remove}"
...
scope_entries_json="$(jq -c --argjson requested "$requested_scope_values_json" '
  [.api.oauth2PermissionScopes[]?
    | .value as $value
    | select((.isEnabled // true) and ($requested | index($value)))
    | {id, value}]
' <<<"$api_json")"
missing_scope_values_json="$(jq -nc \
  --argjson requested "$requested_scope_values_json" \
  --argjson found "$(jq -c '[.[].value]' <<<"$scope_entries_json")" \
  '$requested - $found')"

if [ "$(jq -r 'length' <<<"$missing_scope_values_json")" -gt 0 ]; then
  cat >&2 <<MSG
The API app '$api_display_name' ($API_APP_ID) is missing enabled delegated scopes:
$(jq -r '.[] | "  - " + .' <<<"$missing_scope_values_json")
Create or enable every granular scope on the API app registration first, then rerun this script.
Azure Portal path: Microsoft Entra ID > App registrations > $api_display_name > Expose an API > Add a scope.
MSG
  exit 1
fi

scope_uris="$(jq -r --arg prefix "${api_identifier_uri%/}" 'map($prefix + "/" + .value) | join(" ")' <<<"$scope_entries_json")"
scope_ids_json="$(jq -c '[.[].id]' <<<"$scope_entries_json")"
...
echo "Delegated scopes: $scope_uris"
...
current_app="$(az ad app show --id "$gpt_app_id" -o json)"
existing_scope_ids_json="$(jq -c --arg api "$API_APP_ID" '
  [(.requiredResourceAccess // [])[]
    | select(.resourceAppId == $api)
    | .resourceAccess[]?
    | select(.type == "Scope")
    | .id]
' <<<"$current_app")"
...
Token URL: $token_url
Scopes (space separated): $scope_uris
...
3. Confirm every configured granular delegated scope is listed: $scope_uris
MSG
fi
