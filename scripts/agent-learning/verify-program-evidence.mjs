...
export const ACCEPTANCE_RUNTIME_HOSTS = Object.freeze({
...
const MAX_CANDIDATE_FILE_BYTES = 512 * 1024;
...
export function verifyArtifactArchiveDigest(archive, authenticatedDigest, recordedDigest, label = 'artifact') {
  ...
  if (computed !== authenticatedDigest || computed !== recordedDigest) {
...
async function responseText(response, maximumBytes, label) {
...
export function createTrustedGithubClient({ repository, token, fetchImpl = fetch, spawn = spawnSync } = {}) {
  if (repository !== 'JueZ/api') throw new Error('trusted program verification is repository-bound to JueZ/api');
  if (!token) throw new Error('trusted program verification requires the built-in GitHub token');
  const base = `https://api.github.com/repos/${repository}`;
  const headers = {
    Accept: 'application/vnd.github+json',
    Authorization: `Bearer ${token}`,
    'User-Agent': 'JueZ-api-trusted-program-evidence',
  ...
  async function getBoundFile(path, ref, allowedPaths, label) {
    ...
    if (!Number.isSafeInteger(record.size) || record.size < 1 || record.size > MAX_CANDIDATE_FILE_BYTES) {
      throw new Error(`candidate file size is invalid: ${path}`);
    }
    const content = Buffer.from(String(record.content || '').replaceAll('\n', ''), 'base64');
    if (content.length !== record.size || content.length > MAX_CANDIDATE_FILE_BYTES) {
  ...
  async function getPullRequestFiles(prNumber) {
    ...
    for (let page = 1; page <= 30; page += 1) {
      const rows = await getJson(`/pulls/${prNumber}/files?per_page=100&page=${page}`);
  ...
  function downloadArtifact(artifactId) {
    ...
    const completed = spawn('gh', ['api', `repos/${repository}/actions/artifacts/${artifactId}/zip`], {
      ...
      maxBuffer: MAX_LEDGER_ARCHIVE_BYTES + 1,
      timeout: 30_000,
    });
    if (completed.status !== 0 || !Buffer.isBuffer(completed.stdout)) {
      throw new Error('authenticated artifact download failed');
    }
    if (completed.stdout.length < 1 || completed.stdout.length > MAX_LEDGER_ARCHIVE_BYTES) {
...
async function readLedgerArchive(archive, environment, spawn = spawnSync) {
  const directory = await mkdtemp(join(tmpdir(), 'trusted-agent-learning-ledger-'));
  ...
  try {
    ...
    const expectedEntry = `release-ledger-${environment}.json`;
    if (entries.length !== 1 || entries[0] !== expectedEntry) {
      throw new Error(`${environment} ledger archive must contain only ${expectedEntry}`);
    }
    ...
    if (extracted.status !== 0) throw new Error(`${environment} ledger archive could not be read`);
    return parseStrictJson(extracted.stdout, `${environment} release ledger`);
  } finally {
...
export async function fetchAllowedRuntimeHealth(environment, apiBaseUrl, fetchImpl = fetch) {
  ...
  const response = await fetchImpl(`${origin}/health`, {
    headers: { Accept: 'application/json', 'User-Agent': 'JueZ-api-trusted-program-evidence' },
    redirect: 'error',
    signal: AbortSignal.timeout(20_000),
  });
  if (!response.ok) throw new Error(`${environment} live health returned HTTP ${response.status}`);
  return parseStrictJson(await responseText(response, MAX_HEALTH_BYTES, `${environment} live health`), 'live health');
}

async function collectPhase2Observed(evidence, client, dependencies = {}) {
  const fetchHealth = dependencies.fetchHealth || fetchAllowedRuntimeHealth;
  const readArchive = dependencies.readArchive || readLedgerArchive;
  const implementation = evidence.implementation.pullRequest;
  ...
  await Promise.all(
    aggregates.map(async (record) => {
      observed.checkRuns[String(record.checkRunId)] = await client.getJson(`/check-runs/${record.checkRunId}`);
    }),
  );
  await Promise.all(
    workflowRunIds.map(async (runId) => {
      observed.workflowRuns[String(runId)] = await client.getJson(`/actions/runs/${runId}`);
    }),
  );
...
export async function verifyTrustedPullRequest(options, dependencies = {}) {
  ...
  if (review?.risk?.highRisk === true) {
    ...
    const claimMarker = await client.getJson(`/check-runs/${review.reviewClaim.checkRunId}`);
  ...
  const evidenceText = await client.getFile(PHASE_2_EVIDENCE_PATH, options.headSha);
