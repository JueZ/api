...
test('trusted controller workflow SHA must be authenticated as stable protected-main history', () => {
  ...
  const main = { ref: 'refs/heads/main', object: { type: 'commit', sha: currentMainSha } };
  ...
  assert.ok(
    protectedMainControllerFindings(main, main, { ...comparison, merge_base_commit: { sha: headSha } }, options).some(
      (finding) => finding.includes('comparison merge base'),
    ),
  );
  assert.ok(
    protectedMainControllerFindings(
      main,
      { ref: 'refs/heads/main', object: { type: 'commit', sha: headSha } },
      comparison,
      options,
    ).some((finding) => finding.includes('changed')),
  );
  assert.ok(
...
test('trusted verification rejects provider-shaped governance evidence before candidate collection', async (context) => {
  ...
  const client = {
    async getProtectedMainRef() {
      ...
      return { ref: 'refs/heads/main', object: { type: 'commit', sha: currentMainSha } };
    },
    async compareControllerToMain() {
      requests.push(`/compare/${mergeSha}...${currentMainSha}`);
      return protectedMainComparison();
    },
