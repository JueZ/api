...
test('repair parser finds PRs, workflow run URLs, run IDs, commit SHAs, and environment', () => {
  const parsed = parseRepairIssueBody(
    `Production failure fixed by PR #169. Run https://github.com/JueZ/api/actions/runs/123 for ${sha}`,
  );
...
test('repair decision does not close production issue merely because a PR merged', () => {
  const decision = decideRepairIssueAction(
    { title: 'Production smoke failed', body: 'PR #1 merged' },
    { prStates: [{ merged: true, checksPassed: true }] },
  );
  ...
  const decision = decideRepairIssueAction(
    { title: 'Production smoke failed', body: '' },
    {
      productionVerification: {
        ...
        validationErrors: [],
        liveHealth: { deployedCommitSha: sha },
      },
    },
  );
  assert.equal(decision.action, 'close');
...
test('repair decision leaves production issue open when auth smoke is blocked or failed', () => {
  ...
  const decision = decideRepairIssueAction(
    { title: 'Production smoke failed', body: '' },
    { productionVerification: { ledger: blockedLedger, workflowConclusion: 'success', validationErrors: [] } },
  );
  assert.equal(decision.action, 'comment');
});
  ...
  const decision = decideRepairIssueAction(
    { title: 'Production smoke failed', body: '' },
    {
      productionVerification: {
        ...
        workflowConclusion: 'success',
        validationErrors: [],
        liveHealth: { deployedCommitSha: 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' },
      },
    },
  );
  assert.equal(decision.action, 'comment');
...
test('repair duplicate marker logic is stable per issue and decision kind', () => {
  assert.equal(
    hasDuplicateTriageComment(
      [{ body: '<!-- codex-repair-triage:7:resolved-production-verification -->\nreason' }],
      7,
      'resolved-production-verification',
      'reason',
    ),
    true,
  );
