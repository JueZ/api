...
- Resolve the immediately previous production SHA from live `/health`, then select exactly one retained immutable release bundle and matching successful production ledger for that SHA.
- Accept only a successful first-attempt `Delivery v2` run on protected `main` with exact repository, source SHA, correlation, release artifact, and production ledger identity.
- If production already reports the previous SHA, record `production-unchanged` and do not mutate it.
