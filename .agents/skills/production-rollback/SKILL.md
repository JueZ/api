...
- An active repository security deployment hold blocks rollback before cloud access. Incident containment takes precedence over availability; do not add an emergency input, variable, historical-ref, or local CLI bypass. The active-only record cannot be cleared inside GitHub while no independent approver/trust root exists; external revocation and an out-of-band security bootstrap are required.
